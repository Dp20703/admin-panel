...
function Main() {
    ...
    const [complaints, setComplaints] = useState([
        ...
        },
        {
            ...
            status: "Resolved",
            response: "Your payment issue has been resolved. Please check your email."
        }
    ]);
    ...
    const handleResponseSubmit = (id) => {
        setComplaints(
            complaints.map((complaint) =>
                complaint.id === id ? { ...complaint, response: responses[id], status: "Resolved" } : complaint
            )
        );
    ...
    const handleStatusUpdate = (id) => {
        setComplaints(
            complaints.map((complaint) =>
                complaint.id === id ? { ...complaint, status: statusUpdates[id] || complaint.status } : complaint
            )
        );
    };
                                    ...
                                    complaints.map((complaint) => (
                                        <div key={complaint.id} className="well">
                                            ...
                                            <p>
                                                ...
                                                <span className={`badge ${complaint.status === "Pending" ? "bg-warning" : complaint.status === "Resolved" ? "bg-success" : "bg-danger"}`}>
                                                    {complaint.status}
                                                </span>
                                            </p>
                                            ...
                                            <div className="mt-3">
                                                <select
                                                    ...
                                                    value={statusUpdates[complaint.id] || complaint.status}
