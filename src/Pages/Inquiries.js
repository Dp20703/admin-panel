...
function Main() {
    ...
    const [inquiries, setInquiries] = useState([
        {
            ...
            message: "I want to know if there are any available apartments in downtown.",
            status: "Pending",
            response: ""
        },
        {
    ...
    const handleResponseSubmit = (id) => {
        setInquiries(
            inquiries.map((inquiry) =>
                inquiry.id === id ? { ...inquiry, response: responses[id], status: "Resolved" } : inquiry
            )
        );
    ...
    const handleStatusUpdate = (id) => {
        setInquiries(
            inquiries.map((inquiry) =>
                inquiry.id === id ? { ...inquiry, status: statusUpdates[id] || inquiry.status } : inquiry
            )
        );
    };
                                    ...
                                    inquiries.map((inquiry) => (
                                        <div key={inquiry.id} className="well">
                                            ...
                                            <p>
                                                ...
                                                <span className={`badge ${inquiry.status === "Pending" ? "bg-warning" : inquiry.status === "Resolved" ? "bg-success" : "bg-danger"}`}>
                                                    {inquiry.status}
                                                </span>
                                            </p>
                                            ...
                                            {inquiry.response ? (
                                                <p><strong>Admin Response:</strong> {inquiry.response}</p>
                                            ) : (
                                                <div>
                                                    <textarea
